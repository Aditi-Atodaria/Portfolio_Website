...
export default function AditiPortfolio() {
  ...
  return (
    <div className={`transition-colors duration-500 ${isDarkMode ? "bg-black text-white" : "bg-white text-black"}`}>
      ...
      {/* ── HERO — full viewport, solid bg with shader only inside rectangle ── */}
      <section className={`relative w-full h-screen overflow-hidden ${isDarkMode ? "bg-black" : "bg-white"}`}>
        ...
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center px-6">
          ...
          {/* Text ABOVE the rectangle — fully readable on solid bg */}
          <p className={`font-mono text-xs tracking-[0.5em] uppercase mb-4 ${isDarkMode ? "text-white/50" : "text-black/50"}`}>
            B.Tech Computer Science · CGPA 8.06
          </p>

          {/* The rectangle: shader fills it, name sits on top */}
          <div
          ...
          >
            ...
            {/* Name — centered inside rectangle, readable over shader */}
            <div className="absolute inset-0 flex items-center justify-center">
              <h1 className={`font-mono font-normal text-5xl md:text-7xl lg:text-8xl leading-tight tracking-tighter ${isDarkMode ? "text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]" : "drop-shadow-[0_2px_12px_rgba(255,255,255,0.4)]"}`}
                style={isDarkMode ? {} : { color: "hsl(220, 25%, 22%)" }}
              >
                ADITI<br />ATODARIA
              </h1>
            </div>
          ...
          {/* Text BELOW the rectangle — fully readable on solid bg */}
          <p className={`font-mono text-sm tracking-[0.35em] uppercase mt-4 ${isDarkMode ? "text-white/50" : "text-black/50"}`}>
        ...
        <button
          onClick={scrollDown}
          className={`absolute bottom-10 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2 font-mono text-xs tracking-widest uppercase transition-opacity ${
      ...
      <div ref={contentRef}>
        ...
        <section id="about" className={`border-t ${isDarkMode ? "border-white/8" : "border-black/8"}`}>
          <div className="max-w-5xl mx-auto px-8 py-24">
            {/* Top row: label + education header */}
            <div className="grid grid-cols-1 md:grid-cols-[3fr_2fr] gap-16 mb-0">
              <p className={`font-mono text-sm tracking-[0.4em] uppercase mb-8 ${isDarkMode ? "text-white/25" : "text-black/60"}`}>01 — About</p>
              <p className={`font-mono text-sm tracking-[0.4em] uppercase mb-8 hidden md:block ${isDarkMode ? "text-white/25" : "text-black/60"}`}>Education</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-[3fr_2fr] gap-16">
              <div>
                <h2 className={`font-mono text-3xl md:text-4xl leading-tight mb-8 ${isDarkMode ? "text-white" : "text-black"}`}>
                  Curious.<br />End-to-End.<br />Builder.
                </h2>
                <p className={`font-mono text-base leading-loose ${isDarkMode ? "text-white/55" : "text-black/80"}`}>
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className={`border-t ${isDarkMode ? "border-white/8" : "border-black/8"} pt-6 space-y-4`}>
                  ...
                  <div className={`flex justify-between font-mono text-sm ${isDarkMode ? "text-white/35" : "text-black/65"}`}>
                    <span>CGPA: 8.06</span>
        ...
        <section id="projects" className={`border-t ${isDarkMode ? "border-white/8" : "border-black/8"}`}>
          <div className="max-w-5xl mx-auto px-8 py-24">
            <p className={`font-mono text-sm tracking-[0.4em] uppercase mb-8 ${isDarkMode ? "text-white/25" : "text-black/60"}`}>02 — Projects</p>
            <h2 className={`font-mono text-3xl md:text-4xl leading-tight mb-16 ${isDarkMode ? "text-white" : "text-black"}`}>
              ...
              {PROJECTS.map((p, i) => (
                <div key={i} className={`border-t transition-colors ${isDarkMode ? "border-white/8" : "border-black/8"}`}>
                  ...
                  <button
                    ...
                    className={`w-full text-left grid grid-cols-[auto_1fr_auto] items-center gap-4 py-6 transition-colors group ${
                      isDarkMode ? "hover:bg-white/2" : "hover:bg-black/2"
                    }`}
                  >
                    <span className={`font-mono text-sm ${isDarkMode ? "text-white/20" : "text-black/50"}`}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <div className="flex items-center gap-4">
                      <span className={`font-mono text-lg ${isDarkMode ? "text-white" : "text-black"}`}>{p.name}</span>
                      <span className={`font-mono text-xs border px-2 py-0.5 rounded-sm ${
                  ...
                  <div className={`overflow-hidden transition-all duration-500 ease-in-out ${
                    openProject === i ? "max-h-[600px] opacity-100" : "max-h-0 opacity-0"
                  }`}>
                    <div className={`ml-9 mb-8 pb-8 border-b ${isDarkMode ? "border-white/8" : "border-black/8"}`}>

                      {/* Subtitle */}
                      <p className={`font-mono text-xs tracking-[0.3em] uppercase mb-4 ${isDarkMode ? "text-white/35" : "text-black/55"}`}>
                        {p.subtitle}
                      </p>

                      {/* Description */}
                      <p className={`font-mono text-sm leading-loose mb-6 max-w-2xl ${isDarkMode ? "text-white/60" : "text-black/75"}`}>
                        {p.desc}
                      </p>
                      ...
                      <ul className="space-y-2 mb-8">
                        {p.bullets.map((b, j) => (
                          <li key={j} className="flex items-start gap-3">
                            ...
                            <span className={`font-mono text-sm leading-relaxed ${isDarkMode ? "text-white/70" : "text-black/80"}`}>
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="flex flex-wrap gap-2 flex-1">
                          {p.stack.map((s, j) => (
                            <span key={j} className={`font-mono text-xs px-2.5 py-1 border rounded-sm ${
                        ...
                        <a
                          ...
                          className={`flex items-center gap-2 font-mono text-xs px-4 py-2 border transition-colors flex-shrink-0 ${
                            isDarkMode
        ...
        <section id="skills" className={`border-t ${isDarkMode ? "border-white/8" : "border-black/8"}`}>
          <div className="max-w-5xl mx-auto px-8 py-24">
            <p className={`font-mono text-sm tracking-[0.4em] uppercase mb-8 ${isDarkMode ? "text-white/25" : "text-black/60"}`}>03 — Skills</p>
            <h2 className={`font-mono text-3xl md:text-4xl leading-tight mb-16 ${isDarkMode ? "text-white" : "text-black"}`}>
              Technical<br />Stack
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
              <div>
                <p className={`font-mono text-sm tracking-[0.3em] uppercase mb-6 ${isDarkMode ? "text-white/25" : "text-black/60"}`}>Languages &amp; Tools</p>
                <div className="flex flex-wrap gap-2">
                  {SKILLS.map((s, i) => (
                    <span
                      key={i}
                      className={`font-mono text-sm px-3 py-1.5 border tracking-wider transition-colors cursor-default ${
                        isDarkMode
              ...
              <div>
                <p className={`font-mono text-sm tracking-[0.3em] uppercase mb-6 ${isDarkMode ? "text-white/25" : "text-black/60"}`}>Currently Exploring</p>
                <div className="space-y-6">
                  {CURRENTLY_EXPLORING.map((item, i) => (
                    <div key={i} className={`border-b pb-6 ${isDarkMode ? "border-white/6" : "border-black/15"}`}>
                      <div className="flex items-center gap-3 mb-2">
                        <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${isDarkMode ? "bg-white/50" : "bg-black/50"}`} />
                        <div className={`font-mono text-base ${isDarkMode ? "text-white/80" : "text-black/90"}`}>{item.title}</div>
                      </div>
                      <div className={`font-mono text-sm leading-relaxed pl-4 ${isDarkMode ? "text-white/35" : "text-black/60"}`}>{item.desc}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <section id="contact" className={`border-t ${isDarkMode ? "border-white/8" : "border-black/8"}`}>
          <div className="max-w-5xl mx-auto px-8 py-24">
            <p className={`font-mono text-sm tracking-[0.4em] uppercase mb-8 ${isDarkMode ? "text-white/25" : "text-black/60"}`}>04 — Contact</p>
            <h2 className={`font-mono text-3xl md:text-4xl leading-tight mb-16 ${isDarkMode ? "text-white" : "text-black"}`}>
              Let's Build<br />Something.
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
              <p className={`font-mono text-base leading-loose ${isDarkMode ? "text-white/50" : "text-black/80"}`}>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className={`flex gap-6 items-baseline border-b pb-4 ${isDarkMode ? "border-white/6" : "border-black/15"}`}>
                    <span className={`font-mono text-sm w-16 flex-shrink-0 tracking-wider ${isDarkMode ? "text-white/25" : "text-black/55"}`}>{link.label}</span>
                    <a
                      href={link.href}
                      target="_blank"
                      rel="noreferrer"
                      className={`font-mono text-base transition-opacity ${isDarkMode ? "text-white/60 hover:text-white" : "text-black/75 hover:text-black"}`}
                    >
                      {link.val}
                    </a>
                  </div>
